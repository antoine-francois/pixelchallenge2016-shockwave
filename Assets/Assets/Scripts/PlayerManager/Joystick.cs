...
public class Joystick
{
    ...
    {
        return Input.GetAxis(_tName2Key[name] + id);
    }

    public static float GetAxisRaw(string name, int id)
    {
        return Input.GetAxisRaw(_tName2Key[name] + id);
    }

    public static bool GetButtonDown(string name, int id)
    {
        return Input.GetButtonDown(_tName2Key[name] + id);
    }

    public static bool GetButton(string name, int id)
    {
        return Input.GetButton(_tName2Key[name] + id);
    }

    public static bool GetButtonUp(string name, int id)
    {
        return Input.GetButtonUp(_tName2Key[name] + id);
    }
}
